...
async function uploadPdfToDrive(
  ...
  const invoicesFolderId = await getOrCreateFolder(folderName); // Root folder
  const yearFolderId = await getOrCreateFolder(year.toString(), invoicesFolderId);
  ...
  const nameFolderId = await getOrCreateFolder(name, dayFolderId);

  // Step 2: Upload file into `name` folder
  const fileMetadata = {
    name: fileName,
    parents: [nameFolderId],
  };
  const media = {
...
async function uploadFileToDrive(
  ...
  filePath,
  fileName,
  mimeType, // <-- Added this to handle different file types dynamically
  year,
  month,
  day,
  name,
  ...
  // Upload file into `name` folder
  const fileMetadata = {
    name: fileName,
    parents: [nameFolderId],
  };
  const media = {
    mimeType: mimeType, // <-- Use the dynamic mimeType here
...
module.exports = { uploadPdfToDrive,uploadFileToDrive };
